...
async def _persist_session_name(llm_service: LLMService, session_id: str, user_message: str) -> None:
...
async def maybe_name_session(llm_service: LLMService, session_id: str, session_name: str, messages: list) -> None:
    ...
    if await session_repository.claim_name(session_id, _build_placeholder(first_user_msg)):
        spawn_background_task(_persist_session_name(llm_service, session_id, first_user_msg))
